...
public:
  Adder(int a, int b) {
    ...
    thread_ =
        std::make_unique<AutoJoinThread>(std::thread(std::move(task), a, b));
  }
  int Get() {
    thread_->join();
    return result_->get();
...
void Foo::Calc(int value, int value2) {
  ...
  std::packaged_task<void(int, int)> task3([this](int a, int b) {
    std::cerr << "Multiply thread started with a: " << a << " b: " << b
              << std::endl;
    std::this_thread::sleep_for(1s);
    value_ = a * b;
  });
  AutoJoinThread add_thread3(
      std::thread(std::move(task3), add_result, add_result2));
}
