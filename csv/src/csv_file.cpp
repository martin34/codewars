...
void ReplaceColumnWithNameInTable(Lines::const_iterator column_to_replace,
                                  std::string const &replacement,
                                  Lines const &headline, Table &table);
Lines JoinTableRows(Table const &table);
Lines CreateCopyWithReplacedColumn(Lines const &in, std::string column_name,
  ...
  auto column_name_in_headline =
      std::find(headline.cbegin(), headline.cend(), column_name);
  if (ColumnNameNotInHeadline(headline, column_name_in_headline)) {
    return in;
  }
  ReplaceColumnWithNameInTable(column_name_in_headline, replacement, headline,
                               table.value());
...
Lines JoinTableRows(Table const &table) {
  Lines output;
  std::transform(table.cbegin(), table.cend(), std::back_inserter(output),
                 [](auto const &line) {
                   std::stringstream ss;
                   for (auto col : line) {
                     ss << col << ',';
...
bool TableIsEmpty(Table const &table) { return table.cbegin() == table.cend(); }
