...
template <typename T> T GetConcreteType(Serializable *data) {
  T *my_data{dynamic_cast<T *>(data)};
  if (my_data) {
    return *my_data;
  } else {
    std::cerr << "Expected type of subscriber does not match received data type"
              << std::endl;
    throw std::bad_cast();
  }
}
...
public:
  ...
  template <typename T> void AddCallback(std::function<void(T)> callback) {
...
private:
  Port port_{};
  std::shared_ptr<SubscriberImp>
      impl; // Created once and pointer passed around, when instance of this is
            // copied or moved.
};
