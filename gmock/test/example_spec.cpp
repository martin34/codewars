...
TEST(GMockExampleSpec, CheckForReturnValue) {
  MockParameterServer server;
  EXPECT_CALL(server, GetValue(Eq("parameter_foo")))
...
TEST(GMockExampleSpec, CheckForException) {
  MockParameterServer server;
  EXPECT_CALL(server, GetValue(Eq("parameter_foo")))
      .WillRepeatedly(Throw(std::runtime_error("bla")));
  EXPECT_THROW(server.GetValue("parameter_foo"), std::runtime_error);
}

TEST(GMockExampleSpec, CheckForCustomType) {
  MockParameterServer server;
  EXPECT_CALL(server, GetValueIfAvailable(("parameter_foo")))
      .Times(1)
      .WillRepeatedly(Return(MyOptional{true}));
  EXPECT_THAT(server.GetValueIfAvailable("parameter_foo"),
              Eq(MyOptional{true}));
}
